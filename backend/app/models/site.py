...
class Site(Base):
    ...
    __table_args__ = (
        # 复合索引：用户ID + 软删除（常用查询：获取用户未删除的站点）
        Index("ix_sites_user_id_is_deleted", "user_id", "is_deleted"),
        # 复合索引：用户ID + 状态（常用查询：按状态过滤用户站点）
        Index("ix_sites_user_id_status", "user_id", "status"),
    )
